...
| **[cite.review](https://cite.review)** | 驗證引用真實性：在投稿前發現虛構案例 | 投稿前 |

建議工作流程：使用 Zotero Perma Archiver 建立研究資料庫 → 用 PermaDrop 封存剩餘網址 → 用 SupraDrop 找出引用錯誤 → 用 cite.review 驗證引用 → 投稿。
